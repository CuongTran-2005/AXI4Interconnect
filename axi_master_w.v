...
//================== REQUEST RAM ==================//
    ...
    output                              w_req,        //request to control
    output                              w_busy,
    input                               w_ram_access,
...
//================ REG W =================//
    ...
    always @(*) begin
        case(state_w)
            ...
            AW:
                if (m_AWVALID_o && m_AWREADY_i && w_ram_access)
                    next_state_w = WDATA;
                else
                    if (m_AWVALID_o && m_AWREADY_i && !w_ram_access)
                        next_state_w = WAIT;
            ...
            WDATA:
                if (m_WVALID_o && m_WREADY_i && m_WLAST_o) //ghi data cuoi, RLAST =1
                    ...
                    if (!w_ram_access)
                        next_state_w = WAIT;  //bi mat quyen truy cap RAM
                    else
    ...
    //assign strobe   = ((1 << bytes_per_beat) - 1) << offset;
	 assign ram_strobe = (reg_set_AWSIZE_i == 0) ? 4'b0001 :
						  (reg_set_AWSIZE_i == 1) ? 4'b0011 :
						  (reg_set_AWSIZE_i == 2) ? 4'b1111 : 4'b1111;
