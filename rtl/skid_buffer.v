...
generate
    if(SBUF_TYPE == 0) begin : FULL_REGISTERED
        ...
        // -- Output
        assign bwd_ready_o      = bwd_ready_q;
        assign bwd_ready_d      = ~(inter_fifo_counter == 2'd2) & ~inter_fifo_almost_full & ~inter_fifo_full; //counter == 2 nghia la gan full, ready xuong 0, prev van la 1 nen ghi vao them 1 lan nx thi full la prev xuong 0 luon
        ...
        assign inter_fifo_wr_en = bwd_handshake & ((~inter_fifo_empty) | (~fwd_handshake));  //da co bwd handshake. Neu co fwd hs va fifo emty (fisrt data) thi cho fwd_data_o la bwd_q luon ko can cho vao fifo, neu hs va ko emty (not fisrt) thi data_o phai la tu fifo lua data_i hien tai vao fifo
        assign inter_fifo_rd_en = fwd_handshake; //co fwd hs thi lay data tu fifo ra ouput, neu fifo empty thi lay tu bwd_q ra, neu fifo emty thi fifo ko lam j
        // -- Common
